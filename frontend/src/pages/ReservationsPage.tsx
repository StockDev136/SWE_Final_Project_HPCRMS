...
export default function ReservationsPage() {
  ...
  async function handleVerifyIdentity(e: React.FormEvent, id: number) {
    ...
    setActionLoading(true);
    try {
      await verifyIdentity({ reservationId: id, licenseNumber, dateOfBirth, licenseExpirationDate, selfieConfirmed });
      setVerifyingId(null);
      setLicenseNumber("");
      setDateOfBirth("");
  ...
  if (loading) return <p className="text-slate-500">Loading your reservations…</p>;

  return (
    <div>
      <h1 className="text-2xl font-bold text-[#122A4D] mb-6">My Reservations</h1>

      {error && (
        <div className="mb-4 text-sm text-red-700 bg-red-50 border border-red-200 rounded-md px-3 py-2">{error}</div>
      )}
      {actionError && (
        <div className="mb-4 text-sm text-red-700 bg-red-50 border border-red-200 rounded-md px-3 py-2">{actionError}</div>
      )}

      {reservations.length === 0 && !error && (
        <p className="text-slate-500 text-sm">
          You don&apos;t have any reservations yet.{" "}
          <button onClick={() => navigate("/vehicles")} className="text-[#FF6B35] font-medium underline">
            Search vehicles
          </button>{" "}
          to get started.
        </p>
          ...
          <div key={r.id} className="bg-white rounded-xl shadow-sm p-5">
            <div className="flex items-start justify-between flex-wrap gap-2 mb-3">
              <div>
                ...
                <h3 className="font-bold text-[#122A4D]">
                  {r.vehicle ? `${r.vehicle.make} ${r.vehicle.model}` : "Vehicle pending assignment"}
                </h3>
            ...
            <div className="flex items-center justify-between text-sm mb-3">
              <span className="text-slate-600">
                {r.status === "COMPLETED" && r.finalCost !== null ? (
                  <>Final cost: <span className="font-semibold text-[#122A4D]">${r.finalCost}</span></>
                ) : (
                  <>Est. cost: <span className="font-semibold text-[#122A4D]">${r.estimatedCost}</span></>
                )}
                {r.paid && <span className="ml-2 text-emerald-600 font-semibold">Paid ✓</span>}
              </span>
              {r.status === "PENDING" && (
                <span className={`text-xs font-semibold ${r.identityVerified ? "text-emerald-600" : "text-amber-600"}`}>
            ...
            {r.pickupCode && r.status === "READY_FOR_PICKUP" && (
              <div className="mb-3 bg-blue-50 border border-blue-100 rounded-md px-3 py-2">
                ...
                {r.paid && !isPickupDateReached(r.startDate) && (
                  <p className="text-xs text-blue-600 mt-1">Save this code — pickup opens on {formatDate(r.startDate)}</p>
                )}
              </div>
            )}
            ...
            {verifyingId === r.id && (
              <form onSubmit={(e) => handleVerifyIdentity(e, r.id)} className="mt-4 pt-4 border-t border-slate-100 space-y-3">
                <p className="text-xs text-slate-500">
                  Simulated identity check — no real License Verification Service is connected yet, but the
                  license, age, and expiration checks below are real validations.
                </p>
                {verifyError && (
            ...
            {checkingInId === r.id && (
              <form onSubmit={(e) => handleCheckIn(e, r.id)} className="mt-4 pt-4 border-t border-slate-100 space-y-3">
                <div>
                ...
                <label className="flex items-start gap-2 text-sm text-slate-700">
                  ...
                  />
                  <span>I have read and agree to the Rental Agreement terms above</span>
                </label>
                <div>
                  <label className="block text-xs font-medium text-slate-700 mb-1">
                    Sign by typing your full legal name (as on file: {r.customerName})
                  </label>
                  <input
                    required
                    value={signature}
                  ...
                  {signature.length > 0 && !isValidFullName(signature) && (
                    <p className="text-xs text-red-600 mt-1">Please enter your full first and last name</p>
                  )}
                </div>
                <div className="flex gap-2">
                  <button
            ...
            {payingId === r.id && (
              <form onSubmit={(e) => handlePay(e, r.id)} className="mt-4 pt-4 border-t border-slate-100 space-y-3">
                {paymentError && (
                  <div className="text-xs text-red-700 bg-red-50 border border-red-200 rounded-md px-2 py-1.5">
                    {paymentError}
                  </div>
